    ...
    export const OrderCardDash=({ order, name, onChangeStatus}: OrderCardProps)=>{
        return (
           <div className={`m-3 flex-col justify-start rounded-3xl ${order.status=='open' ? "bg-[#6b3d40]" :order.status == 'in_progress'?"bg-amber-300/30": "bg-[#6f9265]/30"} border-4 ${order.status =='open'? "border-[#360303]" :order.status == 'in_progress'?"border-yellow-500": "border-[#142e0d]"} w-56 h-75 p-2`}>
