...
export function CardListManager({ orders, clients, onChangeStatus ,onDeleteOrder}: CardListProps) {return(
<div className=" ml-10 w-95/100 h-220">
    ...
    <div className=" p-3 gap-3 m-3 flex flex-wrap justify-start rounded-xl bg-[#12282d]/70 border border-[#3a5d65] w-full min-h-100 max-h-200 overflow-y-auto">{orders.map((currentOrder) =>{
        ...
        return (
        <OrderCardManager key={currentOrder.id} order={currentOrder} name={name} onChangeStatus={onChangeStatus} onDeleteOrder={onDeleteOrder}/>
    )})}
    </div>
</div>
)
